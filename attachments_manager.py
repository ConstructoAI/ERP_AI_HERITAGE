...
class AttachmentsManager:
    ...
    def _get_file_storage_path(self, filename: str) -> str:
        """Génère le chemin de stockage du fichier avec normalisation"""
        current_year = datetime.now().year
        current_month = datetime.now().month
        monthly_dir = os.path.join(self.base_upload_dir, str(current_year), f"{current_month:02d}")
    ...
    def debug_attachment_preview(self, attachment_id: int) -> Dict:
        """Fonction de debug pour diagnostiquer les problèmes d'aperçu"""
        debug_info = {
        ...
        try:
            # Vérifier la pièce jointe en base
            attachment = self.get_attachment_by_id(attachment_id)
            if attachment:
                debug_info['attachment_found'] = True
                debug_info['file_path'] = attachment['file_path']
                debug_info['file_extension'] = attachment.get('file_extension', 'Unknown')
                
                # Vérifier si le fichier existe
                if os.path.exists(attachment['file_path']):
                    debug_info['file_exists'] = True
                    debug_info['file_size'] = os.path.getsize(attachment['file_path'])
                    ...
                    try:
                        ...
                        debug_info['error_details'].append(f"Fichier non lisible: {e}")
                        
                    # Vérifier si prévisualisable
                    debug_info['previewable'] = self.is_file_previewable(attachment['file_extension'])
                else:
                    debug_info['error_details'].append(f"Fichier non trouvé: {attachment['file_path']}")
            else:
                debug_info['error_details'].append(f"Pièce jointe {attachment_id} non trouvée en base")
                
        except Exception as e:
            debug_info['error_details'].append(f"Erreur debug: {e}")
    ...
    def get_attachment_by_id(self, attachment_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if result:
                attachment = dict(result[0])
                # Normaliser le chemin du fichier
                if 'file_path' in attachment and attachment['file_path']:
                    attachment['file_path'] = os.path.normpath(attachment['file_path'])
                return attachment
            return None
    ...
    def preview_attachment(self, attachment_id: int) -> Optional[Dict]:
        ...
        try:
            attachment = self.get_attachment_by_id(attachment_id)
            if not attachment:
                st.error(f"❌ Pièce jointe {attachment_id} introuvable")
                return None
            
            file_path = attachment['file_path']
            if not os.path.exists(file_path):
                st.error(f"❌ Fichier physique non trouvé : {file_path}")
                st.info("🔍 Vérifiez que le fichier n'a pas été supprimé du système")
                return {
...
def show_file_preview_modal(attachments_manager: AttachmentsManager, attachment_id: int):
    ...
    debug_mode = st.checkbox("🔧 Mode debug", key=f"debug_mode_{attachment_id}")
    
    if debug_mode:
        st.markdown("### 🔍 Diagnostic de l'aperçu")
        debug_info = attachments_manager.debug_attachment_preview(attachment_id)
        
        col1, col2 = st.columns(2)
        with col1:
            st.write("**État du fichier :**")
            st.write(f"• Pièce jointe trouvée: {'✅' if debug_info['attachment_found'] else '❌'}")
            st.write(f"• Fichier existe: {'✅' if debug_info['file_exists'] else '❌'}")
            st.write(f"• Fichier lisible: {'✅' if debug_info['file_readable'] else '❌'}")
            
        with col2:
            st.write("**Informations :**")
            st.write(f"• Extension: {debug_info['file_extension']}")
            st.write(f"• Prévisualisable: {'✅' if debug_info['previewable'] else '❌'}")
            st.write(f"• Taille: {attachments_manager.format_file_size(debug_info['file_size'])}")
            
        if debug_info['file_path']:
            st.write(f"**Chemin**: `{debug_info['file_path']}`")
    ...
    if not preview_data:
        ...
        if not debug_mode:
            st.info("💡 Activez le mode debug ci-dessus pour plus d'informations")
        return
    ...
    if error:
        ...
        try:
            # Normaliser le chemin du fichier pour compatibilité Windows/Unix
            content = os.path.normpath(content)
            
            # Vérifier que le fichier existe
            if not os.path.exists(content):
                st.error(f"❌ Fichier PDF non trouvé : {content}")
                st.info(f"🔍 Chemin normalisé : {content}")
                return
            
            # Méthode 1 : Streamlit native (recommandée)
            with open(content, 'rb') as pdf_file:
                pdf_bytes = pdf_file.read()
            
            if len(pdf_bytes) == 0:
                st.error("❌ Fichier PDF vide")
                return
            
            # Utiliser st.download_button avec preview pour PDF
            st.markdown("---")
            
            # Créer le bouton avec preview intégré
            col1, col2 = st.columns([3, 1])
            
            with col1:
                st.info("📋 **Aperçu du PDF** : Cliquez sur 'Ouvrir le PDF' pour visualiser le contenu complet")
                
                # Afficher informations du PDF
                file_size = len(pdf_bytes)
                st.markdown(f"📊 **Taille** : {attachments_manager.format_file_size(file_size)}")
                
            with col2:
                # Bouton d'ouverture dans nouvel onglet
                import base64
                pdf_base64 = base64.b64encode(pdf_bytes).decode('utf-8')
                
                # Créer un lien d'ouverture
            ...
            # Méthode alternative : Utilisation native Streamlit
            st.markdown("---")
            st.markdown("**📑 Aperçu intégré :**")
            
            # Option 1: Essayer l'embed HTML
            try:
                embed_html = f"""
                <div style="width: 100%; height: 500px; border: 1px solid #ccc; border-radius: 5px; overflow: hidden;">
                    <iframe 
                        src="data:application/pdf;base64,{pdf_base64}#view=FitH" 
                        width="100%" 
                        height="500px"
                        frameborder="0"
                        style="border: none;">
                        <p>Navigateur non compatible avec l'aperçu PDF intégré.</p>
                    </iframe>
                </div>
                """
                ...
                # Message d'aide
                st.info("💡 Si l'aperçu ne s'affiche pas, utilisez le bouton '🔍 Ouvrir le PDF' ci-dessus")
                
                # Option alternative : Streamlit PDF download button avec aperçu
                st.markdown("---")
                ...
                with col_alt2:
                    ...
                    st.info(f"📊 **{attachments_manager.format_file_size(len(pdf_bytes))}**")
                    
                    # Essayer d'extraire le nombre de pages
                    try:
                        import PyPDF2
                        pdf_reader = PyPDF2.PdfReader(io.BytesIO(pdf_bytes))
                        st.write(f"📖 **{len(pdf_reader.pages)} pages**")
                    except:
                        st.write("📖 **PDF**")
                
            except Exception as embed_error:
                st.warning(f"Aperçu intégré non disponible : {embed_error}")
                
                # Fallback: Afficher juste les métadonnées
                st.markdown("**📋 Informations du fichier :**")
                st.write(f"• **Nom** : {attachment['original_filename']}")
                st.write(f"• **Taille** : {attachments_manager.format_file_size(len(pdf_bytes))}")
                st.write(f"• **Type** : Document PDF")
                
                # Tenter d'extraire quelques infos PDF si possible
                try:
                    import PyPDF2
                    pdf_reader = PyPDF2.PdfReader(io.BytesIO(pdf_bytes))
                    st.write(f"• **Pages** : {len(pdf_reader.pages)}")
                    
                    # Premier paragraphe si possible
                    if len(pdf_reader.pages) > 0:
                        first_page = pdf_reader.pages[0]
                        text_preview = first_page.extract_text()[:200]
                        if text_preview.strip():
                            st.markdown("**📄 Aperçu du contenu :**")
                            st.text_area("", value=text_preview + "...", height=100, disabled=True)
                        
                except ImportError:
                    st.info("💡 Pour un meilleur aperçu PDF, installez PyPDF2: `pip install PyPDF2`")
                except Exception:
                    pass
            
        except Exception as e:
            st.error(f"Erreur lors de l'affichage du PDF: {e}")
            st.info("💡 Utilisez le bouton de téléchargement pour ouvrir le PDF localement")
