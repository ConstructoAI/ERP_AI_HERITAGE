...
# Types d'entreprises construction Québec
TYPES_ENTREPRISES_CONSTRUCTION = [
...
# Secteurs construction spécialisés
SECTEURS_CONSTRUCTION = [
...
class GestionnaireCRM:
    ...
    def _create_demo_data_sqlite(self):
        ...
        try:
            ...
            # Créer entreprises de démonstration CONSTRUCTION QUÉBEC
            entreprises_demo = [
                {
                    'id': 101,
                    'nom': 'Constructions Laval Inc.',
                    'secteur': 'Construction résidentielle',
                    'type_entreprise': 'Entrepreneur général',
                    'adresse': '1550 Boul. Saint-Martin Ouest',
                    'ville': 'Laval',
                    'province': 'Québec',
                    'code_postal': 'H7S 1N4',
                    'pays': 'Canada',
                    'site_web': 'constructions-laval.qc.ca',
                    'notes': 'Projet maisons neuves secteur Fabreville. RBQ actif.'
                },
                {
                    'id': 102,
                    'nom': 'Excavation Dynamique',
                    'secteur': 'Excavation et terrassement',
                    'type_entreprise': 'Sous-traitant spécialisé',
                    'adresse': '2845 Route 132',
                    'ville': 'Longueuil',
                    'province': 'Québec',
                    'code_postal': 'J4L 1H7',
                    'pays': 'Canada',
                    'site_web': 'excavation-dynamique.ca',
                    'notes': 'Partenaire excavation. Équipements lourds disponibles.'
                },
                {
                    'id': 103,
                    'nom': 'Toitures Expert Québec',
                    'secteur': 'Couverture et toiture',
                    'type_entreprise': 'Sous-traitant spécialisé',
                    'adresse': '890 Rue Industrielle',
                    'ville': 'Québec',
                    'province': 'Québec',
                    'code_postal': 'G1M 3S4',
                    'pays': 'Canada',
                    'site_web': 'toitures-expert.qc.ca',
                    'notes': 'Spécialiste toitures résidentielles. Licence RBQ couvreur.'
                },
                {
                    'id': 104,
                    'nom': 'Habitations Prestige Montréal',
                    'secteur': 'Construction haut de gamme',
                    'type_entreprise': 'Promoteur immobilier',
                    'adresse': '3200 Rue Jean-Talon Est',
                    'ville': 'Montréal',
                    'province': 'Québec',
                    'code_postal': 'H2A 1W8',
                    'pays': 'Canada',
                    'site_web': 'habitations-prestige.com',
                    'notes': 'Condos de luxe. Projets 500k$+ par unité.'
                },
                {
                    'id': 105,
                    'nom': 'Rénovations Pro Sherbrooke',
                    'secteur': 'Rénovation résidentielle',
                    'type_entreprise': 'Entrepreneur général',
                    'adresse': '1256 Rue King Ouest',
                    'ville': 'Sherbrooke',
                    'province': 'Québec',
                    'code_postal': 'J1H 1R4',
                    'pays': 'Canada',
                    'site_web': 'reno-pro-sherb.ca',
                    'notes': 'Spécialiste rénovations maisons centenaires.'
                },
                {
                    'id': 106,
                    'nom': 'Plomberie Gatineau Plus',
                    'secteur': 'Plomberie et chauffage',
                    'type_entreprise': 'Sous-traitant spécialisé',
                    'adresse': '445 Boul. Maloney Est',
                    'ville': 'Gatineau',
                    'province': 'Québec',
                    'code_postal': 'J8P 1E6',
                    'pays': 'Canada',
                    'site_web': 'plomberie-gatineau.ca',
                    'notes': 'CCQ actif. Thermopompes et géothermie.'
                }
            ]
            
            for entreprise in entreprises_demo:
                # D'abord vérifier si la colonne type_entreprise existe
                try:
                    self.db.execute_update('''
                        INSERT OR REPLACE INTO companies 
                        (id, nom, secteur, type_entreprise, adresse, ville, province, code_postal, pays, site_web, notes, created_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        entreprise['id'],
                        entreprise['nom'],
                        entreprise['secteur'],
                        entreprise.get('type_entreprise', ''),
                        entreprise['adresse'],
                        entreprise['ville'],
                        entreprise['province'],
                    ...
                    # Si la colonne type_entreprise n'existe pas, l'ajouter
                    if "no column named type_entreprise" in str(e).lower():
                        self.db.execute_update("ALTER TABLE companies ADD COLUMN type_entreprise TEXT")
                        ...
                        # Autre erreur, utiliser l'ancienne méthode
                        self.db.execute_update('''
                            INSERT OR REPLACE INTO companies 
...
# Dans crm.py, à l'intérieur de la classe GestionnaireCRM
    ...
    def ajouter_entreprise(self, data_entreprise):
        ...
        try:
            ...
            base_columns = ['nom', 'type_entreprise', 'secteur', 'site_web', 'contact_principal_id', 'notes', 'created_at', 'updated_at']
...
def render_crm_entreprise_form(crm_manager: GestionnaireCRM, entreprise_data=None):
    ...
    with st.expander(form_title_e, expanded=True):
        with st.form(key="crm_entreprise_form_in_expander", clear_on_submit=False):
            ...
            # Sélection du type d'entreprise construction
            current_type_index = 0
            if entreprise_data and entreprise_data.get('type_entreprise'):
                try:
                    current_type_index = TYPES_ENTREPRISES_CONSTRUCTION.index(entreprise_data['type_entreprise'])
                except ValueError:
                    current_type_index = 0
            
            type_entreprise_e = st.selectbox(
                "Type d'entreprise *",
                options=TYPES_ENTREPRISES_CONSTRUCTION,
                index=current_type_index,
                help="Sélectionnez le type d'entreprise de construction"
            )
            
            # Sélection du secteur construction
            current_secteur_index = 0
            if entreprise_data and entreprise_data.get('secteur'):
                try:
                    current_secteur_index = SECTEURS_CONSTRUCTION.index(entreprise_data['secteur'])
                except ValueError:
                    current_secteur_index = 0
            
            secteur_e = st.selectbox(
                "Secteur d'activité construction *",
                options=SECTEURS_CONSTRUCTION,
                index=current_secteur_index,
                help="Sélectionnez le secteur de construction spécialisé"
            )
            ...
            if submitted_e:
                if not nom_e:
                    ...
                    new_entreprise_data = {
                        'nom': nom_e, 
                        'type_entreprise': type_entreprise_e,
                        'secteur': secteur_e, 
                        'adresse': adresse_e,
                        'ville': ville_e,
                        'province': province_e,
