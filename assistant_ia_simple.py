...
class AssistantIASimple:
    ...
    def __init__(self, db=None, api_key: Optional[str] = None):
        ...
        if self.api_key:
            try:
                ...
                self.model = "claude-sonnet-4-20250514"  # Modèle pour conversation naturelle
                logger.info(f"✅ Assistant IA initialisé avec succès (clé: {self.api_key[:15]}...)")
        ...
        if "mode_conversation" not in st.session_state:
            st.session_state.mode_conversation = "conversation"  # Mode par défaut: conversation naturelle
    ...
    def conversation_naturelle(self, message_utilisateur: str, contexte_projet: Optional[str] = None) -> str:
        ...
        try:
            ...
            donnees_erp = self._fouiller_donnees_erp_completes(contexte_projet)
            
            # Détecter l'intention conversationnelle
            intention = self._detecter_intention_conversation(message_utilisateur)
            
            # Traitement spécial pour création de projet
            if intention == 'creer_projet':
                return self._gerer_creation_projet(message_utilisateur)
            
            # Traitement spécial pour modification de projet
            if intention == 'modifier_projet':
                return self._gerer_modification_projet(message_utilisateur)
            
            # Traitement spécial pour modification de projets multiples
            if intention == 'modifier_projets_multiple':
                return self._gerer_modification_projets_multiples(message_utilisateur)
            
            # Construire le prompt conversationnel
            prompt = self._construire_prompt_naturel(message_utilisateur, donnees_erp, intention, contexte_projet)
            
            # Appel à Claude pour conversation naturelle
            response = self.client.messages.create(
                model=self.model,
                max_tokens=1500,
                temperature=0.8,  # Plus créatif pour conversation naturelle
                messages=[{
                    "role": "user",
                    "content": prompt
                }]
    ...
    def _fouiller_donnees_erp_completes(self, contexte_projet: Optional[str] = None) -> Dict[str, Any]:
        """Fouille complètement les données ERP pour une conversation riche"""
        donnees = {}
        ...
        try:
            # 1. DONNÉES PROJETS GLOBALES - Version sécurisée
            try:
                projets_actifs = self.db.execute_query("""
                    SELECT p.*, c.nom as client_nom
                    FROM projects p
                    LEFT JOIN companies c ON p.client_company_id = c.id
                    WHERE p.statut IN ('EN COURS', 'À FAIRE', 'VALIDÉ')
                    ORDER BY p.created_at DESC
                    LIMIT 10
                """)
            except Exception as e:
                # Fallback si la requête échoue
                logger.error(f"Erreur requête projets: {e}")
                projets_actifs = self.db.execute_query("""
                    SELECT * FROM projects 
                    WHERE statut IN ('EN COURS', 'À FAIRE', 'VALIDÉ')
                    LIMIT 10
                """)
            donnees['projets_actifs'] = [dict(p) for p in projets_actifs]
            
            # 2. PROJET SPÉCIFIQUE SI MENTIONNÉ
            if contexte_projet:
                projet_details = self._get_projet_complet(contexte_projet)
                if projet_details.get('info'):
                    donnees['projet_specifique'] = projet_details
            
            # 3. ÉQUIPE ET EMPLOYÉS - Version sécurisée
            try:
                employes = self.db.execute_query("""
                ...
                logger.error(f"Erreur requête employés: {e}")
                employes = []
            donnees['employes'] = [dict(e) for e in employes]
            
            # 4. INVENTAIRE ET ALERTES - Version sécurisée
            try:
                ...
                logger.error(f"Erreur requête inventaire: {e}")
                stocks_bas = []
            donnees['alertes_stock'] = [dict(s) for s in stocks_bas]
            
            # 5. ACTIVITÉ CRM RÉCENTE - Version sécurisée
            try:
                clients_actifs = self.db.execute_query("""
                    SELECT * FROM companies
                    ORDER BY created_at DESC
                    LIMIT 10
                """)
            except Exception as e:
                logger.error(f"Erreur requête companies: {e}")
                clients_actifs = []
            donnees['clients_actifs'] = [dict(c) for c in clients_actifs]
            ...
            try:
                stats_globales = self.db.execute_query("""
                    ...
                    FROM projects
                """)
                donnees['stats_globales'] = dict(stats_globales[0]) if stats_globales else {}
            except Exception as e:
                logger.error(f"Erreur requête stats: {e}")
                donnees['stats_globales'] = {}
            
            return donnees
            
        except Exception as e:
            logger.error(f"Erreur fouille données ERP: {e}")
            return {"erreur": str(e)}
    
    def _get_projet_complet(self, contexte_projet: str) -> Dict[str, Any]:
        """Récupère tous les détails d'un projet spécifique"""
        try:
            # Info principale du projet
            try:
                projet_info = self.db.execute_query("""
                    SELECT p.*, c.nom as client_nom
                    FROM projects p
                ...
                logger.error(f"Erreur requête projet: {e}")
                # Fallback sans JOIN
                try:
                    projet_info = self.db.execute_query("""
                        SELECT * FROM projects
                        WHERE id = ? OR nom_projet LIKE ?
                    """, (contexte_projet, f"%{contexte_projet}%"))
                except Exception as e2:
                    logger.error(f"Erreur requête projet fallback: {e2}")
                    return {}
            ...
            try:
                ...
                logger.error(f"Erreur requête opérations: {e}")
                operations = []
            ...
            try:
                ...
                logger.error(f"Erreur requête pointages: {e}")
                pointages = []
            
            # Calcul avancement
            avancement = self._calculer_avancement_simple(projet['id'])
            
            return {
                'info': projet,
                'operations': [dict(o) for o in operations] if operations else [],
                'pointages_recents': [dict(p) for p in pointages],
                'avancement': avancement
            }
            
        except Exception as e:
            logger.error(f"Erreur récupération projet complet: {e}")
            return {"erreur": str(e)}
    
    def _extraire_infos_creation_projets_multiples(self, message: str) -> List[Dict[str, Any]]:
        """Extrait les informations de création de projets multiples depuis un message utilisateur"""
        projets = []
        import re
        
        # Format: CLIENT (NOM PROJET) LOC NGUYEN XUAN (760 RUE STRAVINSKI) PATRICK MICHAUD (505 RUE DES MARGUERITES)
        # Pattern pour extraire chaque paire client/projet
        pattern = r'([A-Z][A-Z\s]+?)\s*\(([^)]+)\)'
        matches = re.findall(pattern, message)
        
        for client_nom, projet_nom in matches:
            client_nom = client_nom.strip()
            projet_nom = projet_nom.strip()
            
            # Éviter les mots-clés comme "CLIENT", "NOM PROJET"
            if client_nom.upper() not in ['CLIENT', 'NOM PROJET', 'PROJET'] and len(client_nom) > 2:
                projet_info = {
                    'nom_client': client_nom,
                    'nom_projet': projet_nom,
                    'adresse': projet_nom,  # Le nom du projet est souvent l'adresse
                    'budget': None,
                    'description': f"Projet créé via IA pour {client_nom}",
                    'priorite': 'MOYEN',
                    'date_prevu': None
                }
                projets.append(projet_info)
        
        return projets
    
    def _extraire_infos_creation_projet(self, message: str) -> Dict[str, Any]:
        """Extrait les informations de création d'un projet unique depuis un message utilisateur"""
        infos = {
            'nom_client': None,
            'nom_projet': None, 
            'adresse': None,
        ...
        # Patterns: "client va être X", "pour X", "client: X", "client X"
        import re
        ...
        patterns_client = [
            ...
            r'client est ([^,\n.]+)', 
            r'client[:\s]+([^,\n.]+)',
            r'pour ([A-Z][a-zéèê]+ [A-Z][a-zéèê]+)',
            r'([A-Z][a-zéèê]+ [A-Z][a-zéèê]+)(?=\s+et le nom)'
        ]
        
        for pattern in patterns_client:
            match = re.search(pattern, message, re.IGNORECASE)
            if match:
                infos['nom_client'] = match.group(1).strip()
                break
        
        # Extraction du nom/adresse du projet
        patterns_projet = [
            r'nom du projet[:\s]+([^,\n.]+)',
            r'projet[:\s]+([^,\n.]+)',
            r'adresse[:\s]+([^,\n.]+)',
        ...
        for pattern in patterns_projet:
            ...
            if match:
                ...
                # Si c'est une adresse avec numéro, c'est probablement le nom du projet
                if re.match(r'\d+', nom_trouve):
                    infos['nom_projet'] = nom_trouve
                    infos['adresse'] = nom_trouve
                else:
                    infos['nom_projet'] = nom_trouve
                break
        
        # Extraction du budget
        budget_matches = re.findall(r'budget[:\s]*(\d+[\d\s,.]*)\$?', message, re.IGNORECASE)
        if budget_matches:
            try:
                budget_str = budget_matches[0].replace(' ', '').replace(',', '')
                infos['budget'] = float(budget_str)
            except:
                pass
        
        # Extraction de la priorité
        if 'urgent' in message_lower or 'priorité' in message_lower:
            infos['priorite'] = 'URGENT'
        elif 'faible' in message_lower or 'basse' in message_lower:
            infos['priorite'] = 'FAIBLE'
        
        # Extraction de la description (reste du message)
        if infos['nom_client'] and infos['nom_projet']:
            # Enlever les infos déjà extraites pour garder le reste comme description
            description = message
            for val in [infos['nom_client'], infos['nom_projet']]:
                if val:
                    description = description.replace(val, '')
            
            # Nettoyer
            description = re.sub(r'(client va être|nom du projet|projet:|client:|adresse:)', '', description, flags=re.IGNORECASE)
            description = description.strip(' ,:.-')
            if len(description) > 10:  # Garder seulement si significatif
                infos['description'] = description
        
        return infos
    
    def _creer_client_automatique(self, nom_client: str) -> Optional[int]:
        """Crée automatiquement un client s'il n'existe pas"""
        if not self.db or not nom_client:
            return None
        
        try:
            # Vérifier si le client existe déjà
            client_existant = self.db.execute_query(
                "SELECT id FROM companies WHERE nom LIKE ?",
                (f"%{nom_client}%",)
            )
            
            if client_existant:
                return client_existant[0]['id']
            
            # Créer nouveau client
            # Séparer prénom/nom si possible
            parties = nom_client.strip().split()
            if len(parties) >= 2:
                prenom = parties[0]
                nom = ' '.join(parties[1:])
            else:
            ...
            # Debug: Vérifier le chemin de la base de données
            logger.info(f"DEBUG: Création client dans DB: {getattr(self.db, 'db_path', 'Path unknown')}")
            
            company_id = self.db.execute_insert("""
                INSERT INTO companies 
                (nom, secteur, type_entreprise, created_at)
                VALUES (?, 'Construction', 'CLIENT', datetime('now'))
            """, (nom_client,))
            
            if company_id:
                logger.info(f"✅ Client créé automatiquement: {nom_client} (ID: {company_id}) dans {getattr(self.db, 'db_path', 'DB inconnue')}")
                
                # Vérification immédiate
                verification = self.db.execute_query("SELECT COUNT(*) as count FROM companies WHERE nom = ?", (nom_client,))
                logger.info(f"DEBUG: Vérification immédiate - Client trouvé: {verification[0]['count'] if verification else 0}")
            
            return company_id
            
        except Exception as e:
            logger.error(f"Erreur création client automatique: {e}")
            return None
    
    def _creer_projet_via_ia(self, infos_projet: Dict[str, Any]) -> Dict[str, Any]:
        """Crée un projet via l'assistant IA"""
        if not self.db:
            return {"success": False, "error": "Base de données non disponible"}
        
        try:
            # 1. Valider les infos essentielles
            if not infos_projet.get('nom_client') or not infos_projet.get('nom_projet'):
                return {
                    "success": False, 
                    "error": "Informations manquantes: nom du client et nom du projet requis"
                }
            
            # 2. Créer/trouver le client
            client_id = self._creer_client_automatique(infos_projet['nom_client'])
            if not client_id:
                return {
                    "success": False,
                    "error": f"Impossible de créer le client: {infos_projet['nom_client']}"
                }
            ...
            donnees_projet = {
                'nom_projet': infos_projet['nom_projet'],
                'client_company_id': client_id,
                'client_contact_id': None,
                'po_client': None,
                'statut': 'À FAIRE',
                'priorite': infos_projet.get('priorite', 'MOYEN'),
                'tache': '1.1 Définir les besoins et objectifs du projet',  # Première tâche
                'date_soumis': datetime.now().isoformat(),
                'date_prevu': infos_projet.get('date_prevu'),
                'bd_ft_estime': 0.0,
                'prix_estime': infos_projet.get('budget', 0.0),
                'description': infos_projet.get('description', f"Projet créé via IA pour {infos_projet['nom_client']}")
            }
            
            # 4. Générer l'ID professionnel et créer le projet
            logger.info(f"DEBUG: Création projet dans DB: {getattr(self.db, 'db_path', 'Path unknown')}")
            next_professional_id = self._get_next_professional_id()
            donnees_projet['id'] = next_professional_id  # Forcer l'ID professionnel
            
            project_id = self.db.create_project(donnees_projet)
            
            if project_id:
                logger.info(f"✅ Projet créé avec succès: ID {project_id} dans {getattr(self.db, 'db_path', 'DB inconnue')}")
                
                # Vérification immédiate
                verification = self.db.execute_query("SELECT COUNT(*) as count FROM projects WHERE id = ?", (project_id,))
                logger.info(f"DEBUG: Vérification immédiate - Projet trouvé: {verification[0]['count'] if verification else 0}")
                ...
                return {
                    ...
                    "client_id": client_id,
                    "nom_projet": infos_projet['nom_projet'],
                    "nom_client": infos_projet['nom_client'],
                    "message": f"\u2705 Projet créé avec succès!\n\n🏢 **Projet:** {infos_projet['nom_projet']}\n👤 **Client:** {infos_projet['nom_client']}\n💼 **Statut:** À FAIRE\n🎢 **Priorité:** {infos_projet.get('priorite', 'MOYEN')}\n\nLe projet est maintenant dans le système avec la première tâche assignée!\n\n🔍 **Debug:** Sauvé dans {getattr(self.db, 'db_path', 'DB inconnue')}"
                }
            else:
                return {
                    "success": False,
                    "error": "Erreur lors de la création du projet dans la base de données"
                }
                
        except Exception as e:
            logger.error(f"Erreur création projet via IA: {e}")
            return {
                "success": False,
                "error": f"Erreur technique: {str(e)}"
            }
    
    def _gerer_creation_projet(self, message_utilisateur: str) -> str:
        """Gère la création d'un ou plusieurs projets via conversation naturelle"""
        try:
            # 1. Essayer d'extraire plusieurs projets d'abord
            projets_multiples = self._extraire_infos_creation_projets_multiples(message_utilisateur)
            
            if len(projets_multiples) > 1:
                # Création de plusieurs projets
                return self._creer_projets_multiples(projets_multiples)
            
            # 2. Sinon, traiter comme un projet unique
            infos = self._extraire_infos_creation_projet(message_utilisateur)
            
            # 3. Validation des informations essentielles
            if not infos['nom_client']:
                return "😅 Pour créer un projet, j'ai besoin au minimum du nom du client. Peux-tu me préciser qui est le client ?"
            
            if not infos['nom_projet']:
                return f"🏢 Parfait pour le client **{infos['nom_client']}** ! Il me faut aussi le nom ou l'adresse du projet. Comment veux-tu appeler ce projet ?"
            
            # 4. Créer le projet unique
            resultat = self._creer_projet_via_ia(infos)
            
            if resultat['success']:
                return resultat['message']
            else:
                return f"😞 Désolé, j'ai eu un problème pour créer le projet : {resultat['error']}\n\nVeux-tu que je réessaie ou que tu ajustes les informations ?"
                
        except Exception as e:
            logger.error(f"Erreur gestion création projet: {e}")
            return f"😅 Oups, j'ai eu un petit problème technique pour créer le projet. Peux-tu réessayer en précisant le nom du client et du projet ?"
    
    def _get_next_professional_id(self) -> int:
        """Génère le prochain ID professionnel dans le format 10000, 10001, etc."""
        if not self.db:
            return 10000
            
        try:
            # Chercher le plus grand ID numérique existant
            ...
            if result and result[0]['id']:
                ...
                next_id = max(max_numeric_id + 1, 10000)
            else:
                next_id = 10000
                
            logger.info(f"🔢 Prochain ID professionnel généré: {next_id}")
            return next_id
            
        except Exception as e:
            logger.error(f"Erreur génération ID professionnel: {e}")
            return 10000
    
    def _creer_projets_multiples(self, projets_info: List[Dict[str, Any]]) -> str:
        """Crée plusieurs projets à la fois"""
        try:
            succes = []
            echecs = []
            
            for info in projets_info:
                resultat = self._creer_projet_via_ia(info)
                if resultat['success']:
                    succes.append(f"✅ **{info['nom_client']}** → {info['nom_projet']}")
                else:
                    echecs.append(f"❌ **{info['nom_client']}** → {resultat['error']}")
            
            # Construire le message de retour
            message = f"🚀 **Création de {len(projets_info)} projets terminée !**\n\n"
            
            if succes:
                message += f"**✅ CRÉÉS AVEC SUCCÈS ({len(succes)}):**\n"
                message += "\n".join(succes) + "\n\n"
            
            if echecs:
                message += f"**❌ ÉCHECS ({len(echecs)}):**\n"
                message += "\n".join(echecs) + "\n\n"
            
            from datetime import datetime
            message += f"🕐 **Traitement terminé:** {datetime.now().strftime('%d/%m/%Y à %H:%M')}"
            
            if succes:
                message += f"\n\n🎯 **Tous les projets créés sont en statut 'À FAIRE' avec la première tâche assignée !**"
            
            return message
            
        except Exception as e:
            logger.error(f"Erreur création projets multiples: {e}")
            return f"😅 Oups, j'ai eu un problème technique pour créer les projets multiples : {str(e)}"
    
    def _extraire_infos_modification_projet(self, message: str) -> Dict[str, Any]:
        """Extrait les informations de modification de projet depuis un message utilisateur"""
        infos = {
            'identifiant_projet': None,  # Nom, adresse, ID, client
            'nouveau_statut': None,
            'nouvelle_priorite': None,
            'nouvelle_tache': None,
            'nouveau_budget': None,
            'nouvelles_dates': None,
            'nouvelle_description': None
        }
        
        import re
        message_lower = message.lower()
        
        # 1. EXTRACTION DE L'IDENTIFIANT DU PROJET
        patterns_projet = [
            r'projet\s+([^,\n]+?)(?=\s+en|\s+à|\s+statut|\s+priorité)',
            r'le projet\s+([^,\n]+?)(?=\s+en|\s+à|\s+statut|\s+priorité)',
            r'(\d+\s+rue\s+[^,\n]+)',
            r'(\d+\s+avenue\s+[^,\n]+)',
            r'(\d+\s+boulevard\s+[^,\n]+)',
        ...
        for pattern in patterns_projet:
            match = re.search(pattern, message, re.IGNORECASE)
            if match:
                infos['identifiant_projet'] = match.group(1).strip()
                break
        
        # 2. EXTRACTION DU NOUVEAU STATUT
        statuts_valides = {
            'à faire': 'À FAIRE',
            'a faire': 'À FAIRE', 
            'en cours': 'EN COURS',
            'en cour': 'EN COURS',  # Variante courante
            'en_cours': 'EN COURS',
            'encours': 'EN COURS',
            'validé': 'VALIDÉ',
            'validate': 'VALIDÉ',
        ...
        for status_key, status_val in statuts_valides.items():
            if (f'statut {status_key}' in message_lower or 
                f'en {status_key}' in message_lower or 
                f'à {status_key}' in message_lower or
                f'au statut {status_key}' in message_lower or
                f'mettre {status_key}' in message_lower or
                f'passer {status_key}' in message_lower):
                infos['nouveau_statut'] = status_val
                break
        
        # 3. EXTRACTION DE LA NOUVELLE PRIORITÉ
        priorites_valides = {
            'urgent': 'URGENT',
            'urgente': 'URGENT',
            'haute': 'URGENT',
        ...
        for prio_key, prio_val in priorites_valides.items():
            if f'priorité {prio_key}' in message_lower or f'priorité à {prio_key}' in message_lower:
                infos['nouvelle_priorite'] = prio_val
                break
        
        # 4. EXTRACTION DE LA NOUVELLE TÂCHE
        # Pattern pour les tâches numérotées (ex: "1.2", "5.1", etc.)
        tache_patterns = [
            r'tâche\s+(\d+\.\d+[^,\n]*)',
            r'à la tâche\s+(\d+\.\d+[^,\n]*)',
            r'passer à\s+(\d+\.\d+[^,\n]*)',
        ...
        for pattern in tache_patterns:
            match = re.search(pattern, message, re.IGNORECASE)
            if match:
                infos['nouvelle_tache'] = match.group(1).strip()
                break
        
        # 5. EXTRACTION DU NOUVEAU BUDGET
        budget_patterns = [
            r'budget\s+(?:à\s+|de\s+)?(\d+[\d\s,.]*)\s*\$?',
            r'prix\s+(?:à\s+|de\s+)?(\d+[\d\s,.]*)\s*\$?',
            r'(\d+[\d\s,.]*)\s*\$'
        ]
        ...
        for pattern in budget_patterns:
            ...
            if match:
                try:
                    ...
                    infos['nouveau_budget'] = float(budget_str)
                    break
                except:
                    pass
    ...
    def _rechercher_projet(self, identifiant: str) -> Optional[Dict]:
        """Recherche un projet par nom, adresse, client ou ID"""
        if not self.db or not identifiant:
            return None
        ...
        try:
            ...
            projet = self.db.execute_query("""
                ...
                LEFT JOIN companies c ON p.client_company_id = c.id
                WHERE p.id = ? 
                   OR p.nom_projet LIKE ?
                   OR c.nom LIKE ?
                   OR p.client_nom_cache LIKE ?
                LIMIT 1
            """, (identifiant, f"%{identifiant}%", f"%{identifiant}%", f"%{identifiant}%"))
            
            if projet:
                return dict(projet[0])
            
            return None
            
        except Exception as e:
            logger.error(f"Erreur recherche projet: {e}")
            return None
    
    def _modifier_projet_via_ia(self, projet: Dict, modifications: Dict[str, Any]) -> Dict[str, Any]:
        """Modifie un projet via l'assistant IA"""
        if not self.db or not projet:
            return {"success": False, "error": "Projet ou base de données non disponible"}
        ...
        try:
            ...
            if modifications['nouveau_statut']:
                affected = self.db.execute_update(
                    "UPDATE projects SET statut = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (modifications['nouveau_statut'], project_id)
                )
                if affected > 0:
                    modifications_appliquees.append(f"📊 **Statut:** {modifications['nouveau_statut']}")
            
            # 2. MODIFICATION DE LA PRIORITÉ
            if modifications['nouvelle_priorite']:
                affected = self.db.execute_update(
                    "UPDATE projects SET priorite = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (modifications['nouvelle_priorite'], project_id)
                )
                if affected > 0:
                    modifications_appliquees.append(f"🎯 **Priorité:** {modifications['nouvelle_priorite']}")
            
            # 3. MODIFICATION DE LA TÂCHE
            if modifications['nouvelle_tache']:
                affected = self.db.execute_update(
                    "UPDATE projects SET tache = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (modifications['nouvelle_tache'], project_id)
                )
                if affected > 0:
                    modifications_appliquees.append(f"✅ **Tâche:** {modifications['nouvelle_tache']}")
            
            # 4. MODIFICATION DU BUDGET
            if modifications['nouveau_budget']:
                affected = self.db.execute_update(
                    "UPDATE projects SET prix_estime = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (modifications['nouveau_budget'], project_id)
                )
                if affected > 0:
                    modifications_appliquees.append(f"💰 **Budget:** {modifications['nouveau_budget']}$")
            
            # 5. MODIFICATION DE LA DESCRIPTION
            if modifications['nouvelle_description']:
                affected = self.db.execute_update(
                    "UPDATE projects SET description = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                    (modifications['nouvelle_description'], project_id)
                )
                if affected > 0:
                    modifications_appliquees.append(f"📝 **Description mise à jour**")
            ...
            if modifications_appliquees:
                ...
                return {
                    ...
                    "client_nom": projet.get('client_nom', projet.get('client_nom_cache', 'N/A')),
                    "modifications": modifications_appliquees,
                    "message": f"✅ **Projet modifié avec succès !**\n\n🏢 **Projet:** {projet['nom_projet']}\n👤 **Client:** {projet.get('client_nom', projet.get('client_nom_cache', 'N/A'))}\n\n**Modifications appliquées:**\n" + "\n".join(modifications_appliquees) + f"\n\n🕐 **Mis à jour:** {datetime.now().strftime('%d/%m/%Y à %H:%M')}"
                }
            else:
                return {
                    "success": False,
                    "error": "Aucune modification détectée ou applicable"
                }
                
        except Exception as e:
            logger.error(f"Erreur modification projet via IA: {e}")
            return {
                "success": False,
                "error": f"Erreur technique: {str(e)}"
            }
    
    def _gerer_modification_projet(self, message_utilisateur: str) -> str:
        """Gère la modification d'un projet via conversation naturelle"""
        try:
            # 1. Extraire les informations de modification
            modifications = self._extraire_infos_modification_projet(message_utilisateur)
            
            # 2. Validation de l'identifiant du projet
            if not modifications['identifiant_projet']:
                return "🔍 Pour modifier un projet, j'ai besoin de savoir quel projet ! Peux-tu me préciser le nom du projet, l'adresse, ou le nom du client ?"
            
            # 3. Rechercher le projet
            projet = self._rechercher_projet(modifications['identifiant_projet'])
            if not projet:
                return f"😕 Je n'ai pas trouvé le projet **{modifications['identifiant_projet']}**. Peux-tu vérifier le nom ou me donner plus de détails ?"
            
            # 4. Validation qu'il y a au moins une modification
            modifications_demandees = any([
                modifications['nouveau_statut'],
                modifications['nouvelle_priorite'], 
                modifications['nouvelle_tache'],
                modifications['nouveau_budget'],
                modifications['nouvelle_description']
            ])
            
            if not modifications_demandees:
                return f"🏢 J'ai trouvé le projet **{projet['nom_projet']}** ! Mais que veux-tu modifier exactement ? (statut, priorité, tâche, budget...)"
            
            # 5. Appliquer les modifications
            resultat = self._modifier_projet_via_ia(projet, modifications)
            
            if resultat['success']:
                return resultat['message']
            else:
                return f"😞 Désolé, j'ai eu un problème pour modifier le projet : {resultat['error']}"
                
        except Exception as e:
            logger.error(f"Erreur gestion modification projet: {e}")
            return f"😅 Oups, j'ai eu un petit problème technique pour modifier le projet. Peux-tu réessayer en précisant le projet et les modifications ?"
    
    def _gerer_modification_projets_multiples(self, message_utilisateur: str) -> str:
        """Gère la modification de plusieurs projets à la fois"""
        try:
            # 1. Extraire les modifications demandées
            modifications = self._extraire_infos_modification_projet(message_utilisateur)
            
            # 2. Déterminer quels projets modifier
            projets_a_modifier = []
            message_lower = message_utilisateur.lower()
            
            if any(phrase in message_lower for phrase in ['tous ceux que tu a créé', 'tous les projets créés', 'projets récents']):
                # Projets créés récemment (aujourd'hui)
                from datetime import datetime, timedelta
                date_limite = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')
                
                projets_recents = self.db.execute_query("""
                    SELECT p.*, c.nom as client_nom
                    FROM projects p
                    LEFT JOIN companies c ON p.client_company_id = c.id
                    WHERE p.created_at >= ? 
                ...
                if projets_recents:
                    projets_a_modifier = [dict(p) for p in projets_recents]
            
            elif 'tous' in message_lower or 'tout' in message_lower:
                # Tous les projets actifs
                tous_projets = self.db.execute_query("""
                    SELECT p.*, c.nom as client_nom
                    FROM projects p
                    LEFT JOIN companies c ON p.client_company_id = c.id
                ...
                if tous_projets:
                    projets_a_modifier = [dict(p) for p in tous_projets]
            
            # 3. Validation
            if not projets_a_modifier:
                return "🤔 Je n'ai pas trouvé de projets correspondants à modifier. Peux-tu être plus spécifique ?"
            
            if not any([modifications['nouveau_statut'], modifications['nouvelle_priorite'], modifications['nouvelle_tache']]):
                return f"📋 J'ai trouvé {len(projets_a_modifier)} projets à modifier ! Mais que veux-tu changer exactement ? (statut, priorité, tâche...)"
            
            # 4. Appliquer les modifications à tous les projets
            return self._appliquer_modifications_multiples(projets_a_modifier, modifications)
            
        except Exception as e:
            logger.error(f"Erreur gestion modification projets multiples: {e}")
            return f"😅 Oups, j'ai eu un problème technique pour modifier les projets multiples : {str(e)}"
    
    def _appliquer_modifications_multiples(self, projets: List[Dict], modifications: Dict[str, Any]) -> str:
        """Applique les modifications à plusieurs projets à la fois"""
        try:
            succes = []
            echecs = []
            ...
            for projet in projets:
                try:
                    # Appliquer les modifications à ce projet
                    modifications_appliquees = []
                    project_id = projet['id']
                    ...
                    if modifications_appliquees:
                        client_nom = projet.get('client_nom', projet.get('client_nom_cache', 'N/A'))
                        succes.append(f"✅ **{client_nom}** ({projet['nom_projet']}) → {', '.join(modifications_appliquees)}")
                    else:
                        client_nom = projet.get('client_nom', projet.get('client_nom_cache', 'N/A'))
                        echecs.append(f"⚠️ **{client_nom}** ({projet['nom_projet']}) → Aucune modification appliquée")
                        
                except Exception as e:
                    client_nom = projet.get('client_nom', projet.get('client_nom_cache', 'N/A'))
                    echecs.append(f"❌ **{client_nom}** ({projet['nom_projet']}) → Erreur: {str(e)}")
            
            # Construire le message de retour
            message = f"🚀 **Modification de {len(projets)} projets terminée !**\n\n"
            
            if succes:
                message += f"**✅ MODIFIÉS AVEC SUCCÈS ({len(succes)}):**\n"
                message += "\n".join(succes) + "\n\n"
            
            if echecs:
                message += f"**⚠️ PROBLÈMES ({len(echecs)}):**\n"
                message += "\n".join(echecs) + "\n\n"
            ...
            logger.error(f"Erreur application modifications multiples: {e}")
            return f"😅 Erreur technique lors des modifications multiples : {str(e)}"
    
    def _calculer_avancement_simple(self, project_id: int) -> Dict[str, Any]:
        """Calcule un avancement simple du projet"""
        try:
            stats = self.db.execute_query("""
                SELECT 
                    COUNT(*) as total_ops,
                    COUNT(CASE WHEN statut = 'TERMINÉ' THEN 1 END) as ops_terminees,
                    COUNT(CASE WHEN statut = 'EN COURS' THEN 1 END) as ops_en_cours,
                    SUM(temps_estime) as heures_prevues,
                    SUM(CASE WHEN statut = 'TERMINÉ' THEN temps_estime ELSE 0 END) as heures_terminees
                FROM operations
                WHERE project_id = ?
            """, (project_id,))
            ...
            if stats and stats[0]:
                s = stats[0]
                pourcentage = (s['ops_terminees'] / s['total_ops'] * 100) if s['total_ops'] else 0
                return {
                    'pourcentage_operations': round(pourcentage, 1),
                    'operations_terminees': s['ops_terminees'],
                    'operations_totales': s['total_ops'],
                    'operations_en_cours': s['ops_en_cours'],
                    'heures_terminees': s['heures_terminees'],
                    'heures_prevues': s['heures_prevues']
                }
            return {}
        except Exception as e:
            return {"erreur": str(e)}
    
    def _detecter_intention_conversation(self, message: str) -> str:
        """Détecte l'intention dans une conversation naturelle"""
        message_lower = message.lower()
        
        # Détection d'opinions demandées
        if any(mot in message_lower for mot in ['penses quoi', 'ton avis', 'que penses-tu', "qu'en penses-tu"]):
            return 'demande_opinion'
        
        # Détection de demande de statut/situation
        if any(mot in message_lower for mot in ['comment va', 'comment ça va', 'état', 'situation', 'avancement']):
            return 'demande_status'
        
        # Détection de problèmes/risques
        if any(mot in message_lower for mot in ['problème', 'souci', 'risque', 'attention', 'inquiet']):
            return 'detection_problemes'
        
        # Détection de demande de conseils
        if any(mot in message_lower for mot in ['conseil', 'recommande', 'suggère', 'que faire', 'devrais']):
            return 'demande_conseil'
        
        # Détection questions financières
        if any(mot in message_lower for mot in ['budget', 'coût', 'prix', 'rentable', 'argent']):
            return 'analyse_financiere'
        
        # Détection questions équipe
        if any(mot in message_lower for mot in ['équipe', 'employé', 'qui peut', 'compétence', 'disponible']):
            return 'gestion_equipe'
        
        # Détection questions matériel/stock
        if any(mot in message_lower for mot in ['matériel', 'stock', 'inventaire', 'manque', 'commande']):
            return 'gestion_materiel'
        
        # Détection questions temps/planning
        if any(mot in message_lower for mot in ['planning', 'délai', 'retard', 'temps', 'livraison']):
            return 'gestion_temps'
        
        # Détection création de projet
        if any(mot in message_lower for mot in ['créer un projet', 'nouveau projet', 'créer projet', 'créé un projet', 'ajouter projet', 'créer un nouveau', 'je veux que tu créer', 'créer ce projet', 'client :', 'nom du projet :', 'créer ces projets', 'peux-tu me créer']):
            return 'creer_projet'
        
        # Détection modification de projet
        if any(mot in message_lower for mot in ['modifier', 'changer', 'mettre', 'passer à', 'change la', 'modifie', 'mettre le projet', 'change le statut', 'priorité à', 'tâche à']):
            # Vérification pour modification en lot
            if any(mot in message_lower for mot in ['tous', 'tout', 'toutes', 'tous les projets', 'tous ceux', 'toute la liste']):
                return 'modifier_projets_multiple'
            return 'modifier_projet'
        
        return 'question_generale'
    
    def _construire_prompt_naturel(self, message: str, donnees: Dict, intention: str, contexte_projet: Optional[str]) -> str:
        """Construit un prompt pour conversation naturelle avec toutes les données"""
        
        prompt = f"""Tu es l'assistant IA expert de Constructo AI Inc., une entreprise de construction au Québec.

Tu es un collègue virtuel expérimenté et sympathique. Tu connais parfaitement toute l'entreprise et tu parles naturellement avec tes collègues.

PERSONNALITÉ:
- Tu tutoies naturellement (tu/toi)
- Ton amical, professionnel mais décontracté
- Tu donnes ton opinion basée sur les vraies données
- Tu exprimes des préoccupations ou félicitations quand c'est approprié
- Tu restes informatif mais concis
- Tu utilises des emojis avec modération

DONNÉES TEMPS RÉEL DE L'ERP:
...
"""
        ...
        if intention == 'demande_opinion':
            prompt += "\n\nINSTRUCTION: L'utilisateur te demande ton avis. Analyse les données réelles et donne ton opinion d'expert construction, sois franc mais constructif."
        
        elif intention == 'demande_status':
            prompt += "\n\nINSTRUCTION: Donne un point de situation naturel, comme avec un collègue. Mentionne les éléments importants de façon conversationnelle."
        
        elif intention == 'detection_problemes':
            prompt += "\n\nINSTRUCTION: Analyse attentivement les données pour identifier les vrais problèmes ou risques. Sois le collègue vigilant qui repère les soucis."
        
        elif intention == 'demande_conseil':
            prompt += "\n\nINSTRUCTION: Donne des conseils pratiques et actionnables basés sur les données réelles. Sois l'expert qui guide."
        
        if contexte_projet:
            prompt += f"\n\nCONTEXTE: L'utilisateur fait référence au projet '{contexte_projet}'. Concentre ta réponse sur ce projet avec ses vraies données."
        
        prompt += "\n\nRÉPONDS NATURELLEMENT: Comme un collègue expert qui a accès à toutes les données ERP. Sois utile, personnel et informatif."
        
        return prompt
    
    def _extraire_contexte_projet(self, message: str) -> Optional[str]:
        """Extrait le nom d'un projet mentionné dans le message"""
        if not self.db:
            return None
        
        try:
            # Rechercher des mots-clés qui indiquent qu'on parle d'un projet
            mots_cles_projet = ['projet', 'chantier', 'construction', 'maison', 'bâtiment', 'résidence']
            
            # Si un mot-clé projet est trouvé, chercher les noms de projets dans la base
            if any(mot in message.lower() for mot in mots_cles_projet):
                projets = self.db.execute_query("""
                    SELECT nom_projet, id FROM projects 
                    WHERE statut IN ('EN COURS', 'À FAIRE', 'VALIDÉ')
                    ORDER BY LENGTH(nom_projet) DESC
                """)
                
                # Chercher si un nom de projet est mentionné dans le message
                for projet in projets:
                    nom_projet = projet['nom_projet'].lower()
                    if nom_projet in message.lower() or any(mot in nom_projet for mot in message.lower().split() if len(mot) > 3):
                        return str(projet['id'])
            
            return None
        except Exception as e:
            logger.error(f"Erreur extraction contexte projet: {e}")
            return None
    ...
    def _get_claude_response(self, prompt: str, context: Dict = None) -> str:
        ...
        try:
            ...
            system_message = """Tu es un assistant expert spécialisé en gestion ERP pour l'industrie de la construction au Québec.

🏗️ EXPERTISE SPÉCIFIQUE:
- Gestion de projets de construction résidentielle et commerciale au Québec
- Connaissance approfondie des normes RBQ et CCQ
- Maîtrise de la réglementation québécoise en construction
- Expertise en gestion des sous-traitants et main-d'œuvre
- Calcul des taxes TPS/TVQ spécifiques au secteur construction
- Gestion des matériaux et équipements de construction
- Planification et suivi de chantiers

📊 DOMAINES D'EXPERTISE ERP:
- Gestion de projets de construction (phases, étapes, jalons)
- Suivi des bons de travail et assignation de ressources
- Gestion d'inventaire (matériaux, outils, équipements)
- Conformité construction (licences RBQ, cartes CCQ, attestations)
- Calcul de devis et estimation de coûts
- Gestion des employés et sous-traitants
- TimeTracker et suivi des heures par poste de travail
- Rapports de production et analyses de rentabilité

🇨🇦 CONTEXTE QUÉBÉCOIS:
- Utilise la terminologie française du domaine de la construction
- Applique les taux de taxes du Québec (TPS 5%, TVQ 9.975%)
- Respecte les normes du Code de construction du Québec
- Considère les conditions climatiques et saisonnières du Québec
- Mentionne les organismes pertinents (RBQ, CCQ, CNESST, ACQ)

RÈGLE ABSOLUE - SOURCE UNIQUE DE VÉRITÉ:
- Tu DOIS te baser EXCLUSIVEMENT sur les données fournies dans le contexte ERP
- Tu ne dois JAMAIS inventer ou supposer des informations
- Chaque information doit être traçable aux données SQLite
- Si une information n'est pas disponible, indique-le clairement

💡 RECOMMANDATIONS PRATIQUES:
- Propose des optimisations spécifiques à la construction québécoise
- Suggère des améliorations de processus basées sur les meilleures pratiques
- Identifie les risques potentiels (retards, dépassements, conformité)
- Recommande des actions préventives

FORMAT DE RÉPONSE:
- Utilise des tableaux markdown pour les listes
- Structure avec ## et ### pour les sections
- **Gras** pour les éléments importants
- Emojis pertinents: 🏗️ chantier, 📋 devis, 🔨 travaux, 📏 matériaux
- Montants avec espaces: 25 000 $ (format québécois)
- Dates en français: 15 mars 2025

Exemple pour projets de construction:
| **Projet** | **Type** | **Phase** | **Budget** | **Échéance** | **Conformité** |
|------------|----------|-----------|------------|--------------|----------------|
| Résidence Cartier | Résidentiel | Fondations | 125 000 $ | 30 avril 2025 | ✅ RBQ valide |

Réponds comme un expert-conseil en construction au Québec."""
    ...
    def show_page(self):
        ...
        st.markdown("""
        <div class="ia-header">
            <h1>🏗️ Assistant IA - Expert Construction Québec</h1>
            <p>Votre expert en gestion ERP pour l'industrie de la construction au Québec</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.sidebar:
            ...
            with st.expander("💡 Guide d'utilisation"):
                ...
                **🆕 CONVERSATION NATURELLE:**
                Parle-moi comme à un collègue !
                
                **💬 Exemples naturels:**
                - "Tu en penses quoi du projet X ?"
                - "Comment ça va sur le chantier ?"
                - "Y'a-t-il des problèmes ?"
                - "Que me conseilles-tu ?"
                
                **⚡ Commandes rapides:**
                - `/stats` - Statistiques ERP
    ...
    def _process_input(self, user_input: str) -> str:
        """Traite l'input utilisateur avec conversation naturelle par défaut"""
        ...
        # Mode conversation naturelle par défaut (NOUVEAU!)
        elif not input_lower.startswith('/'):
            # Détecter si l'utilisateur mentionne un projet spécifique
            contexte_projet = self._extraire_contexte_projet(user_input)
            
            # Utiliser la nouvelle conversation naturelle
            return self.conversation_naturelle(user_input, contexte_projet)
...
- Expertise en construction résidentielle et commerciale
- Optimisation des processus de chantiers
