...
class PersistentERPDatabase:
    ...
    def setup_environment_detection(self):
        ...
        # Détecter Render (plus robuste)
        self.is_render = (
            os.environ.get('RENDER') is not None or 
            os.path.exists('/opt/render/project')
        ) and not os.name == 'nt'  # Pas sur Windows
        
        # Détecter Hugging Face Spaces
        self.is_huggingface = (
        ...
        if self.is_render:
            ...
            # Sur HF, vérifier si un volume persistant est disponible
            self.has_persistent_disk = (
                os.path.exists('/data') and os.access('/data', os.W_OK)
            )
        else:
            self.has_persistent_disk = True  # Local = persistant
        ...
        logger.info(f"  - Hugging Face: {'✅' if self.is_huggingface else '❌'}")
    ...
    def setup_persistent_paths(self):
        ...
        elif self.is_huggingface and self.has_persistent_disk:
            # Hugging Face avec volume persistant ✅ OPTIMAL
            self.environment_type = "HUGGINGFACE_PERSISTENT"
            self.data_dir = '/data'
            self.db_path = os.path.join(self.data_dir, 'erp_production_dg.db')
            self.backup_dir = os.path.join(self.data_dir, 'backup_json')
            self.storage_status = "PERSISTANT"
            logger.info("🎯 Mode Hugging Face avec stockage persistant ACTIVÉ")
            
        elif self.is_huggingface:
            # Hugging Face sans volume persistant ⚠️ PROBLÉMATIQUE
            self.environment_type = "HUGGINGFACE_EPHEMERAL"
            # Essayer le répertoire de l'app d'abord
            if os.access(os.getcwd(), os.W_OK):
                self.data_dir = os.path.join(os.getcwd(), 'data')
                logger.info("📁 Utilisation répertoire app pour HF")
            else:
                self.data_dir = '/tmp'
                logger.warning("⚠️ Utilisation /tmp pour HF - stockage temporaire")
            
            self.db_path = os.path.join(self.data_dir, 'erp_production_dg.db')
            self.backup_dir = os.path.join(self.data_dir, 'backup_json')
            self.storage_status = "ÉPHÉMÈRE" if '/tmp' in self.data_dir else "SEMI-PERSISTANT"
            
            if '/tmp' in self.data_dir:
                logger.warning("🚨 ATTENTION: Données perdues au redémarrage HF")
                logger.info("💡 Configurez un volume persistant HF pour la persistance")
