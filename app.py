...
from conformite_construction import ConformiteConstruction
...
def get_admin_credentials():
    ...
    return {
        ...
        "constructo_admin": os.environ.get("CONSTRUCTO_ADMIN_PASSWORD", "admin123"),
        "superviseur": os.environ.get("SUPERVISEUR_PASSWORD", "admin123"),
        "direction": os.environ.get("DIRECTION_PASSWORD", "admin123"),
        "production": os.environ.get("PRODUCTION_PASSWORD", "admin123"),
    }
...
def get_user_display_name(username):
    ...
    names = {
        ...
        "constructo_admin": "Admin Constructo AI",
...
class GestionnaireProjetSQL:
    ...
    def diagnostiquer_references_projet(self, projet_id):
        """Diagnostique toutes les références au projet dans la base"""
        projet_id_str = str(projet_id)
        references = {}
        
        # Tables connues qui peuvent référencer un projet
        tables_a_verifier = [
            "project_assignments", "operations", "materials", "time_entries", 
            "formulaires", "formulaire_lignes", "bt_assignations", 
            "bt_reservations_postes", "inventory_history"
        ]
        
        for table in tables_a_verifier:
            try:
                # Vérifier avec project_id
                count_query = f"SELECT COUNT(*) FROM {table} WHERE project_id = ?"
                result = self.db.execute_query(count_query, (projet_id_str,))
                if result and result[0]['COUNT(*)'] > 0:
                    references[f"{table}.project_id"] = result[0]['COUNT(*)']
                
                # Vérifier aussi avec d'autres colonnes possibles
                if table == "formulaire_lignes":
                    # Vérifier via formulaires
                    count_query = f"""
                        SELECT COUNT(*) FROM {table} fl 
                        INNER JOIN formulaires f ON fl.formulaire_id = f.id 
                        WHERE f.project_id = ?
                    """
                    result = self.db.execute_query(count_query, (projet_id_str,))
                    if result and result[0]['COUNT(*)'] > 0:
                        references[f"{table} via formulaires"] = result[0]['COUNT(*)']
                        
            except Exception as e:
                logger.debug(f"Table {table} non trouvée ou erreur: {e}")
    ...
    def supprimer_projet(self, projet_id):
        ...
        try:
            ...
            # Diagnostic initial
            st.info("🔍 Diagnostic des références au projet...")
            references = self.diagnostiquer_references_projet(projet_id)
            
            if references:
                st.warning("📋 Références trouvées :")
                for ref, count in references.items():
                    st.write(f"• {ref}: {count} enregistrement(s)")
            
            # Désactiver temporairement les contraintes de clés étrangères
            try:
                self.db.execute_update("PRAGMA foreign_keys = OFF")
                st.info("🔓 Contraintes de clés étrangères désactivées temporairement")
            except Exception as e:
                st.warning(f"Impossible de désactiver les contraintes FK: {e}")
            
            # Supprimer dans l'ordre correct - seulement les tables qui existent
            tables_suppression = [
                # 1. D'abord les lignes de formulaires (références aux formulaires)
                ("formulaire_lignes via formulaires", """
                    DELETE FROM formulaire_lignes 
                    WHERE formulaire_id IN (
                        SELECT id FROM formulaires WHERE project_id = ?
                    )
                """),
                
                # 2. Ensuite toutes les autres références directes au projet
                ("project_assignments", "DELETE FROM project_assignments WHERE project_id = ?"),
                ("operations", "DELETE FROM operations WHERE project_id = ?"),
                ("materials", "DELETE FROM materials WHERE project_id = ?"),
                ("time_entries direct", "DELETE FROM time_entries WHERE project_id = ?"),
                
                # 3. Finalement les formulaires (après avoir nettoyé leurs dépendances)
                ("formulaires", "DELETE FROM formulaires WHERE project_id = ?"),
            ]
            
            total_supprime = 0
            
            for nom_table, query in tables_suppression:
                try:
                    result = self.db.execute_update(query, (projet_id_str,))
                    if result and result > 0:
                        st.write(f"✅ {nom_table}: {result} enregistrement(s) supprimé(s)")
                        total_supprime += result
                except Exception as e:
                    st.write(f"⚠️ {nom_table}: {e}")

            # Supprimer le projet principal
            result = self.db.execute_update("DELETE FROM projects WHERE id = ?", (projet_id_str,))
            
            # Réactiver les contraintes
            try:
                self.db.execute_update("PRAGMA foreign_keys = ON")
                st.info("🔒 Contraintes de clés étrangères réactivées")
            except Exception as e:
                st.warning(f"Erreur réactivation contraintes FK: {e}")
            
            if result:
                st.success(f"✅ Projet #{projet_id} supprimé avec succès ({total_supprime + 1} enregistrements)")
                return True
            else:
                st.warning(f"⚠️ Projet #{projet_id} introuvable")
                return False
            ...
            try:
                self.db.execute_update("PRAGMA foreign_keys = ON")
                st.info("🔒 Contraintes réactivées après erreur")
            except:
                pass
                
            st.error(f"Erreur suppression projet: {e}")
            logger.error(f"Erreur détaillée suppression projet {projet_id}: {e}")
            return False
    ...
    try:
            ...
            default_companies = [
                {
                    ...
                    'adresse': '123 Rue des Bâtisseurs, Montréal, QC',
                    'site_web': 'www.autotech.com',
                    'notes': 'Client construction maisons haut de gamme'
                },
                {
                    'id': 2,
                    'nom': 'BâtiTech Inc.',
                ...
                },
                {
                    ...
                    'adresse': '789 Ave. des Promoteurs, Laval, QC',
                    'site_web': 'www.aerospace.com',
                    'notes': 'Projets condos et tours d\'habitation'
                }
            ]
        ...
        if work_centers_count == 0:
            ...
            default_work_centers = [
                {
                    'id': 1,
                    'nom': 'Excavation Zone 1',
                    'departement': 'PRODUCTION',
                    'categorie': 'TERRASSEMENT',
                    'type_machine': 'Pelle mécanique',
                    'capacite_theorique': 8.0,
                    'operateurs_requis': 1,
                    'cout_horaire': 150.0,
                    'competences_requises': 'Opération machinerie lourde, CNESST'
                },
                {
                    'id': 2,
                    'nom': 'Fondations Zone 1',
                    'departement': 'STRUCTURE',
                    'categorie': 'BÉTON',
                    'type_machine': 'Coffrage et bétonnière',
                    'capacite_theorique': 10.0,
                    'operateurs_requis': 3,
                    'cout_horaire': 125.0,
                    'competences_requises': 'Coffrage béton, Lecture plans structure'
                },
                {
                    'id': 3,
                    'nom': 'Charpente Zone 1',
                    'departement': 'STRUCTURE',
                    'categorie': 'OSSATURE',
                    'type_machine': 'Poste charpenterie',
                    'capacite_theorique': 8.0,
                    'operateurs_requis': 2,
                    'cout_horaire': 110.0,
                    'competences_requises': 'Charpenterie commerciale, Certification CCQ'
                },
                {
                    'id': 4,
                    'nom': 'Toiture Zone 1',
                    'departement': 'PRODUCTION',
                    'categorie': 'FINITION',
                    'type_machine': 'Poste couverture',
                    'capacite_theorique': 8.0,
                    'operateurs_requis': 2,
                    'cout_horaire': 95.0,
                    'competences_requises': 'Installation toiture, Bardeaux et membrane'
                },
                {
                    'id': 5,
                    'nom': 'Plomberie Zone 1',
                    'departement': 'PRODUCTION',
                    'categorie': 'MÉCANIQUE',
                    'type_machine': 'Poste plomberie',
                    'capacite_theorique': 8.0,
                    'operateurs_requis': 1,
                    'cout_horaire': 120.0,
                    'competences_requises': 'Plomberie certifiée, Code du bâtiment'
                },
                {
                    'id': 6,
                    'nom': 'Électricité Zone 1',
                    'departement': 'PRODUCTION',
                    'categorie': 'MÉCANIQUE',
                    'type_machine': 'Poste électrique',
                    'capacite_theorique': 8.0,
                    'operateurs_requis': 1,
                    'cout_horaire': 130.0,
                    'competences_requises': 'Électricité certifiée, Code électrique'
                },
                {
                    'id': 7,
                    'nom': 'Isolation Zone 1',
                    'departement': 'PRODUCTION',
                    'categorie': 'FINITION',
                    'type_machine': 'Poste isolation',
                    'capacite_theorique': 10.0,
                    'operateurs_requis': 2,
                    'cout_horaire': 85.0,
                    'competences_requises': 'Isolation thermique, Pare-vapeur'
                },
                {
                    'id': 8,
                    'nom': 'Plâtre Zone 1',
                    'departement': 'PRODUCTION',
                    'categorie': 'FINITION',
                    'type_machine': 'Poste plâtrage',
                    'capacite_theorique': 8.0,
                    'operateurs_requis': 2,
                    'cout_horaire': 90.0,
                    'competences_requises': 'Pose gypse, Jointage et finition'
                },
                {
                    'id': 9,
                    'nom': 'Finition Zone 1',
                    'departement': 'PRODUCTION',
                    'categorie': 'FINITION',
                    'type_machine': 'Poste finition générale',
                    'capacite_theorique': 8.0,
                    'operateurs_requis': 2,
                    'cout_horaire': 85.0,
                    'competences_requises': 'Finition intérieure, Peinture et revêtement'
                }
            ]
...
def show_admin_auth():
    ...
    with st.form("admin_login"):
        ...
        username = st.text_input("Nom d'utilisateur:", placeholder="admin, constructo_admin, superviseur, direction, production")
    ...
    with st.expander("🔒 Comptes de Démonstration", expanded=False):
        ...
        **Comptes administrateurs disponibles:**
        ...
        - **constructo_admin** / constructo2024! *(Admin Constructo AI)*
...
def show_erp_main():
    ...
    # 10. Conformité Construction Québec
    if has_all_permissions or "conformite" in permissions:
        available_pages["🏗️ Conformité Construction"] = "conformite_construction"
    ...
    etapes_workflow = {
        ...
        "conformite_construction": "🏗️ Conformité Construction"
    }
    ...
    elif page_to_show_val == "conformite_construction":
        if 'conformite_manager' not in st.session_state:
            st.session_state.conformite_manager = ConformiteConstruction(st.session_state.erp_db)
        st.session_state.conformite_manager.afficher_interface()
    ...
    if stats['total'] > 0 or postes_stats['total_postes'] > 0:
        ...
        with gc2:
            ...
            if postes_stats.get('par_departement'):
                colors_dept = {'PRODUCTION': '#10b981', 'STRUCTURE': '#3b82f6', 'QUALITE': '#f59e0b', 'INGÉNIERIE': '#8b5cf6', 'COMMERCIAL': '#ef4444', 'ADMINISTRATION': '#6366f1', 'DIRECTION': '#f43f5e'}
...
def show_liste_projets():
    ...
    if gestionnaire.projets:
        ...
        with result_col2:
            if projets_filtres:
                ca_filtre = sum(float(str(p.get('prix_estime', 0) or 0).replace(' ', '').replace(',', '').replace('None', '0') or 0) for p in projets_filtres)
...
def render_create_project_form(gestionnaire, crm_manager):
    ...
    with st.form("create_form", clear_on_submit=True):
        ...
        with fc1:
            ...
            # Nouveau menu déroulant pour les contacts
            try:
                contacts_db = st.session_state.erp_db.execute_query("""
                    SELECT c.id, c.nom, c.prenom, comp.nom as entreprise_nom 
                    FROM contacts c 
                    LEFT JOIN companies comp ON c.company_id = comp.id 
                    ORDER BY c.nom, c.prenom
                """)
                liste_contacts_form = [("", "Sélectionner un contact")] + [
                    (c['id'], f"{c['nom']} {c['prenom']}" + (f" ({c['entreprise_nom']})" if c['entreprise_nom'] else ""))
                    for c in contacts_db
                ]
            except Exception as e:
                logger.warning(f"Erreur récupération contacts: {e}")
            ...
            selected_contact_id_form = st.selectbox(
                ...
                format_func=lambda c_id: next((nom for id_c, nom in liste_contacts_form if id_c == c_id), "Sélectionner..."),
                key="project_create_contact_select"
            )
        ...
        if submit:
            ...
            if not nom:
                ...
                if selected_entreprise_id_form:
                    ...
                    # Contact sélectionné
                    contact_check = st.session_state.erp_db.execute_query(
                        "SELECT nom, prenom, company_id FROM contacts WHERE id = ?",
                        (selected_contact_id_form,)
                    )
                    if contact_check:
                        contact = contact_check[0]
                        client_nom_cache_val = f"{contact['nom']} {contact['prenom']}"
                        # Si le contact est lié à une entreprise, utiliser l'ID de l'entreprise
                        if contact['company_id']:
                            client_company_id = contact['company_id']
                    else:
                        st.error(f"Contact ID {selected_contact_id_form} non trouvé en base.")
                        return
...
def render_edit_project_form(gestionnaire, crm_manager, project_data):
    """Formulaire d'édition de projet - Version complète corrigée avec étapes de construction"""
    gestionnaire_employes = st.session_state.gestionnaire_employes
    ...
    with st.form("edit_form", clear_on_submit=True):
        ...
        with fc1:
            ...
            # Menu déroulant pour les contacts (édition)
            try:
                contacts_db = st.session_state.erp_db.execute_query("""
                    SELECT c.id, c.nom, c.prenom, comp.nom as entreprise_nom 
                    FROM contacts c 
                ...
                # Tenter de détecter si le projet actuel utilise un contact
                current_contact_id = ""
                if not current_entreprise_id and project_data.get('client_nom_cache'):
                    # Chercher si le nom correspond à un contact
                    for contact in contacts_db:
                        contact_name = f"{contact['nom']} {contact['prenom']}"
                        if contact_name == project_data.get('client_nom_cache'):
                            current_contact_id = contact['id']
                            break
                            
            except Exception as e:
                logger.warning(f"Erreur récupération contacts pour édition: {e}")
                liste_contacts_form = [("", "Aucun contact disponible")]
                current_contact_id = ""
            ...
            selected_contact_id_form = st.selectbox(
                "Ou Contact direct :",
                options=[c_id for c_id, _ in liste_contacts_form],
                index=next((i for i, (c_id, _) in enumerate(liste_contacts_form) if c_id == current_contact_id), 0),
                format_func=lambda c_id: next((nom for id_c, nom in liste_contacts_form if id_c == c_id), "Sélectionner..."),
                key="project_edit_contact_select"
            )
        ...
        if submit:
            if not nom or (not selected_entreprise_id_form and not selected_contact_id_form and not client_nom_direct_form):
                st.error("Nom du projet et Client obligatoires (sélectionnez une entreprise, un contact, ou saisissez un nom direct).")
            elif d_fin < d_debut:
                st.error("Date fin < date début.")
            else:
                ...
                update_data = {
                    ...
                    'client_company_id': client_company_id,
                    'client_nom_cache': client_nom_cache_val,
                    'client_legacy': client_nom_direct_form if not selected_entreprise_id_form and not selected_contact_id_form and client_nom_direct_form else "",
                    'po_client': po_client,
                    'statut': statut,
                    'priorite': priorite,
                    'tache': tache,
...
def render_delete_confirmation(gestionnaire):
    ...
    if project:
        ...
        st.markdown("Cette action est **irréversible** et supprimera toutes les données associées :")
        st.markdown("- 🧾 Formulaires (Bons de Travail, Bons d'Achat, etc.)")
        st.markdown("- ⏱️ Pointages et time entries")
        st.markdown("- 👥 Assignations employés")
        st.markdown("- 🏭 Réservations postes de travail")
        st.markdown("- 📋 Opérations et matériaux")
        st.markdown("- 📊 Toutes données de production")
