...
class GestionnaireInventaire:
    """
    Gestionnaire d'inventaire connecté à ERPDatabase SQLite - ADAPTÉ CONSTRUCTION QUÉBEC
    Gère les articles, mouvements, alertes et statistiques
    + CONSTRUCTION QUÉBEC : Catégories matériaux, normes CSA/BNQ, unités construction
    """
    
    # Catégories de matériaux construction Québec
    CATEGORIES_MATERIAUX = [
        ...
        "Plomberie - Fixtures",
        "Électricité - Câblage",
        "Électricité - Boîtes et panneaux",
    ...
    # Normes et certifications québécoises
    NORMES_CONSTRUCTION = [
        "CSA A23.1 - Béton",
        "CSA A23.2 - Essais béton", 
        "CSA O86 - Bois d'ingénierie",
        "CSA S16 - Acier de construction",
        "BNQ 2560-114 - Granulats",
        "BNQ 2621-905 - Béton de ciment",
        "CAN/ULC-S701 - Isolants thermiques",
        "CAN/ULC-S703 - Mousse isolante",
        "ASTM - Standards internationaux",
        "Code national du bâtiment"
    ]
    
    def __init__(self, db):
        self.db = db
        logger.info("GestionnaireInventaire initialisé avec base SQLite - Mode Construction Québec")
        self._init_materiaux_construction()
    
    def _init_materiaux_construction(self):
        """Initialise des matériaux de construction québécois si l'inventaire est vide"""
        try:
            # Vérifier si des articles existent déjà
            count_query = "SELECT COUNT(*) as count FROM inventory_items"
            result = self.db.execute_query(count_query)
            
            if result and result[0]['count'] == 0:
                # Liste de matériaux de construction typiques
                materiaux_demo = [
                    {
                        "nom": "Béton 25 MPa",
                        "type": "Béton et ciment",
                        "unite_imp": "vg³",
                        "unite_met": "m³",
                        "conversion": 1.308,
                        "limite_min": 50,
                        "fournisseur": "Béton Provincial",
                        "norme": "CSA A23.1 - Béton"
                    },
                    {
                        "nom": "2x4 Épinette 8 pieds",
                        "type": "Bois de charpente",
                        "unite_imp": "pièce",
                        "unite_met": "pièce", 
                        "conversion": 1,
                        "limite_min": 100,
                        "fournisseur": "Bois Expansion",
                        "norme": "CSA O86 - Bois d'ingénierie"
                    },
                    {
                        "nom": "Contreplaqué 1/2\" 4x8",
                        "type": "Contreplaqué et OSB",
                        "unite_imp": "feuille",
                        "unite_met": "feuille",
                        "conversion": 1,
                        "limite_min": 25,
                        "fournisseur": "Rona L'Entrepôt",
                        "norme": "CSA O121 - Contreplaqué"
                    },
                    {
                        "nom": "Isolant laine R-20",
                        "type": "Isolants thermiques",
                        "unite_imp": "pi²",
                        "unite_met": "m²",
                        "conversion": 10.764,
                        "limite_min": 500,
                        "fournisseur": "Isolofoam",
                        "norme": "CAN/ULC-S701 - Isolants thermiques"
                    },
                    {
                        "nom": "Bardeaux d'asphalte",
                        "type": "Bardeaux et toiture",
                        "unite_imp": "paquet",
                        "unite_met": "paquet",
                        "conversion": 1,
                        "limite_min": 50,
                        "fournisseur": "Toitures BP",
                        "norme": "CSA A123.5 - Bardeaux"
                    },
                    {
                        "nom": "Tuyau ABS 3\"",
                        "type": "Plomberie - Tuyauterie",
                        "unite_imp": "pi",
                        "unite_met": "m",
                        "conversion": 3.281,
                        "limite_min": 100,
                        "fournisseur": "Rona L'Entrepôt",
                        "norme": "CSA B181.1 - Tuyauterie ABS"
                    },
                    {
                        "nom": "Fil électrique 14/2 NMD90",
                        "type": "Électricité - Câblage",
                        "unite_imp": "pi",
                        "unite_met": "m",
                        "conversion": 3.281,
                        "limite_min": 300,
                        "fournisseur": "Rona L'Entrepôt",
                        "norme": "CSA C22.1 - Code électrique"
                    },
                    {
                        "nom": "Clous 3\" galvanisés",
                        "type": "Fixations et ancrages",
                        "unite_imp": "lb",
                        "unite_met": "kg",
                        "conversion": 2.205,
                        "limite_min": 25,
                    ...
                    {
                        "nom": "Pierre concassée 0-3/4\"",
                        "type": "Pierre concassée",
                        "unite_imp": "tonne",
                        "unite_met": "tonne",
                        "conversion": 1,
                        "limite_min": 50,
                        "fournisseur": "Groupe Permacon",
                        "norme": "BNQ 2560-114 - Granulats"
                    },
                    {
                        "nom": "Gypse 1/2\" 4x8 régulier",
                        "type": "Gypse et produits de finition",
                        "unite_imp": "feuille",
                        "unite_met": "feuille",
                        "conversion": 1,
                        "limite_min": 50,
                ...
                # Créer les articles dans l'inventaire
                for mat in materiaux_demo:
                    # Calculer les quantités
                    qte_imperial = mat["limite_min"] * 2  # Stock initial = 2x limite min
                    qte_metric = qte_imperial / mat["conversion"] if mat["unite_imp"] != mat["unite_met"] else qte_imperial
                    
                    item_data = {
                        'nom': mat["nom"],
                        'type_produit': mat["type"],
                        'quantite_imperial': f"{qte_imperial} {mat['unite_imp']}",
                        'quantite_metric': qte_metric,
                        'limite_minimale_imperial': f"{mat['limite_min']} {mat['unite_imp']}",
                        'limite_minimale_metric': mat["limite_min"] / mat["conversion"] if mat["unite_imp"] != mat["unite_met"] else mat["limite_min"],
                        'quantite_reservee_imperial': "0",
                        'quantite_reservee_metric': 0,
                        'description': f"Matériau de construction conforme {mat.get('norme', 'aux normes')}",
                        'notes': f"Unité impériale: {mat['unite_imp']}, Unité métrique: {mat['unite_met']}",
                        'fournisseur_principal': mat["fournisseur"]
                    }
                    
                    self.add_item(item_data)
                
                logger.info("✅ Matériaux de construction québécois initialisés")
                
        except Exception as e:
            logger.error(f"Erreur initialisation matériaux construction: {e}")
...
def render_add_item_tab(inventory_manager):
    ...
    with st.form("add_item_form", clear_on_submit=True):
        ...
        with col1:
            nom = st.text_input("Nom de l'article *:", placeholder="Ex: Béton 30 MPa, 2x4 8 pieds")
            code_interne = st.text_input("Code interne:", placeholder="Auto-généré si vide")
            type_produit = st.selectbox("Type de produit:", 
                [""] + GestionnaireInventaire.CATEGORIES_MATERIAUX,
                help="Sélectionnez la catégorie de matériau construction"
            )
            fournisseur = st.text_input("Fournisseur principal:", placeholder="Ex: Béton Provincial")
        ...
        # Section normes construction
        st.markdown("**Normes et certifications:**")
        norme_applicable = st.selectbox(
            "Norme applicable:", 
            ["Aucune"] + GestionnaireInventaire.NORMES_CONSTRUCTION,
            help="Sélectionnez la norme de construction applicable"
        )
        
        description = st.text_area("Description:", placeholder="Description détaillée du matériau, dimensions, propriétés...")
        notes = st.text_area("Notes:", placeholder="Notes internes, conditions de stockage, précautions...")
        ...
        if submitted:
            if not nom:
                ...
                # Ajouter la norme dans la description ou les notes
                description_complete = description.strip()
                if norme_applicable != "Aucune":
